...
class _AdminOrderUpdateScreenState
  ...
  List<Purchase> get _trackingEligible {
    return _purchases.where((purchase) {
      ...
      final paidRatio = (totalToPay != null && totalToPay > 0)
          ? (totalPaid / totalToPay)
          : 0;
      final typeAllows = purchase.purchaseType == 'down_40' || purchase.purchaseType == 'down_50';
      final percentAllows = (purchase.downPaymentPercent ?? 0) >= 0.4;
      return purchase.hasPaidDownPayment || paidRatio >= 0.4 || typeAllows || percentAllows;
    }).toList();
  }
...
class _TrackingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const Color accent = Color(0xFFFB9324);
    final imageUrl =
        purchase.product?.images?.isNotEmpty == true ? purchase.product!.images!.first : '';
